...
ctdprocessing_command_info = {    
    ...
    "CatchmentsAndStreams": {
        ...
        "shortHelp": (
            ...
            f"<li>{fop}Ignore Depressions:{fcc} When checked (default), the input DEM is used unchanged and neither sink filling nor depression-map generation is performed. Clear this option to create a depression map for the watershed calculation.</li>"
            f"<li>{fop}Ignore Small Depressions:{fcc} When checked (default), the depression map is generated directly from the original DEM. Clear this option to breach smaller isolated depressions before creating the map.</li>"
            f"<li>{fop}Maximum Depression Breach Length:{fcc} Maximum breach channel length in cells when small-depression breaching is enabled (default 50). Larger values can alter larger depressions and increase processing time.</li>"
            f"<li>{fop}Minimum Depression Depth:{fcc} Minimum fill depth used to mark a cell as part of the depression raster (default 0.1). Increase this to suppress small elevation artifacts.</li>"
            ...
            f"<li>{fop}Output Depression Raster:{fcc} Optional raster output containing the depression mask supplied to the watershed calculation.</li>"
            "</ul>"
            "<p>Depression-map generation is optional and computationally expensive. When enabled, the DEM is sink-filled and compared with the working DEM to identify real depressions; the resulting raster is supplied to the watershed calculation so basins remain treated as depressions rather than being forced to drain out. Breaching smaller depressions is an additional, more expensive preprocessing step and is optional.</p>"
            "<h3>Processing Settings</h3>"
            "<ul>"
            f"<li>{ctdprocessing_settingshelp_text}</li>"
            "</ul>"
